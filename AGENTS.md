...
2. **순위·등급·점수·경고색을 표현할 문법을 두지 않는다.** 객관성을 문서가 아니라 스키마로 강제하는 자리다. 모든 객체가 `additionalProperties: false` 이고 어휘가 전부 닫힌 `enum` 인 이유가 이것이다.
   그 대신 **비교를 시각이 맡는다** — 렌더는 subject 를 **한 좌표에 겹친다**(glossary §3.14 원칙 7). 나란히 늘어놓으면 비교를 사람이 머릿속에서 해야 하고, 시각이 맡기로 한 일을 안 하는 것이 된다. 잴 자가 없는 것(참거짓·글)만 나란히 가른다(원칙 9).
...
**띠와 상자로 말하던 것을 글자로 말하게 한다.** 위계는 크기·굵기·색조·여백이 만든다.
테두리·채움·모서리는 각각 「별개의 물건」이라는 뜻이라 역할을 보고만 쓴다.

지키는 것 — 고정 케이스 `장식으로 위계를 만들지 않는다` 와 `쓰는 색이 전부 무채색이다` 가 본다.

| 하지 않는다 | 왜 |
| --- | --- |
| 둥근 상자에 왼쪽 띠를 덧대기 (`border-left`·`inset` 그림자) | 뜻 없는 장식이 위계를 흉내 낸다 |
| 활성·선택을 **밑줄**로 말하기 (`border-bottom`) | 왼쪽 띠를 아래로 옮긴 것뿐이다. 굵기·색조·면으로 말한다 |
| 주석 네 갈래에 서로 다른 시각 주기 | **갈래이지 심각도가 아니다.** 주의가 더 위험해 보이면 안 된다 |
| 덩어리마다 다른 `border-radius` | 모서리는 한 값뿐이고 진짜 컨테이너에만 붙는다 |
| 점선·겹선 장식 · 빗금 텍스처 · 그림자 | 같은 부류 |
| 빈 자리를 점선 상자와 가운데 정렬로 꾸미기 | 틀이 아니라 자리다. 글로 말한다 |
| 아무거나 카드로 만들기 · 배지와 알약 | `stat` 의 subject 카드는 진짜 별개 물건이라 맞다. 주석·범례·표시는 아니다 |
