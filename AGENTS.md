...
- **축(`variants`)은 템플릿이 갖는다.** facet 을 템플릿이 갖는 것과 같은 까닭이다 — subject 마다 갈래가 다르면 견줄 수가 없다. 한 템플릿에 하나뿐이고 없는 것이 기본이다.
- **축이 바뀌어도 어떤 facet 이 서는지는 달라지지 않는다.** 달라지는 것은 `varies` 를 단 필드의 값뿐이다. 조건부 facet 은 여전히 없다.
