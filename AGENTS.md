...
- **그리는 것과 말이 맞는다.** 겹치는 facet 은 전원의 주석을, focus 를 따라 바뀌는 facet 은 그리고 있는 subject 의 주석만 낸다.
...
| **주석 갈래의 색** (`.note-<kind>` 의 `--kind`) | **통용되는 UI 시맨틱을 따른다.** 표시와 갈래 이름에만 닿는다 |
...
| **색을 값이나 글에 직접 칠하기** | 색 있는 값은 `--subject`·`--kind` 선언에만 설 수 있다. 나머지는 그 변수를 거친다 |
...
| **색 있는 값이 `--subject`·`--kind` 선언에만 서는지** — 값에 직접 칠하는 길이 막혔는지 |
...
| 갈래 색·아이콘이 **그 제안서의 우열처럼** 읽히지 않는가 — 이번의 위험이 여기다 |
