...
- **평가를 시각으로 말하지 않는다.** subject 를 가르는 색은 자리 차례로 배정되고 값의 크기와 무관하다. 선은 색과 점선 무늬로 함께 갈린다. 고정 케이스가 기계로 볼 수 있는 만큼 본다 — 나머지는 사람이 본다.

## 시각을 고칠 때

**뷰어의 시각은 참조일 뿐 제품의 정본이 아니다.** 사람이 언어를 만져 보는 자리라 **읽히는 만큼의 시각**을 갖는다.
다만 **평가를 시각으로 말하지 않는다** — 좋고 나쁨·순위·심각도를 뜻하는 색과 기호를 두지 않는다.
...
| **주석 네 갈래에 무게 차이 주기** | **갈래이지 심각도가 아니다.** 아이콘을 주더라도 넷이 같은 크기·같은 선 굵기다 |
| **도메인 아이콘** — 돈·병원·서류 | 언어가 보험을 모른다는 것이 이 저장소의 첫째 규칙이다 |
| **색만으로 가르기** | `line` 의 선은 색과 **점선 무늬**로 함께 갈린다. 무늬를 없애지 않는다 |
...
| 기계가 본다 (`장식으로 위계를 만들지 않는다` · `평가를 시각으로 말하지 않는다` · `subject 색은 값이 아니라 자리로 배정된다`) |
| --- |
| 색이 있는 자리가 **subject 팔레트뿐**이고 나머지 뼈대는 무채색인지 |
| 팔레트가 `TONES` 개이고 `.sub-1..N` 로만 선언되는지 |
| 렌더가 넣는 inline `style` 이 **길이뿐**인지 (값이 시각을 정하지 않는지) |
| 값을 키우거나 차례를 바꿨을 때 색이 **자리를 따라가는지** |
| 주석 네 갈래의 아이콘 속성이 **똑같은지**, 갈래별 CSS 선택자가 없는지 |
| `⚠`·`★`·`1위` 같은 평가 기호와 순위 어휘가 없는지 |
| 왼쪽 띠·밑줄·그림자·모서리 난립·점선 장식이 없는지 |

| **사람이 봐야 한다** |
| --- |
| 고른 색이 좋고 나쁨으로 읽히지 않는가 (색 여섯의 뜻) |
| 주석 넷의 아이콘이 **실제로** 같은 무게로 보이는가 |
