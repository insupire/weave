...
- **제목은 이름, 설명은 `hint`.** facet 도 필드도 `title`/`label` 은 **명사구로 짧게** 쓰고 「이것이 무엇인지」는 `hint` 가 진다. facet 의 hint 는 **늘 보이고** 필드의 것은 **표시를 열면** 보인다 — 뜻은 같고 **밀도** 때문에 갈린다(facet 은 서넛, 필드는 열 개씩).
- **샘플이 본보기다.** 템플릿을 쓰는 쪽이 여기 말투를 따라 쓴다 — 물음 꼴 제목을 두지 않는다. 말투는 기계가 못 재므로 되돌아가기 쉬운 한 가지(물음 꼴)만 판정이 막고 나머지는 사람이 본다.
- **필드에 붙는 글 셋이 독자로 갈린다.** `description` 은 **채우는 쪽에게**(화면에 안 나옴), `hint` 는 **보는 사람에게 이 필드가 무엇인지**(1~80자 한 줄), 값 한 벌의 필드 주석은 **이 값에 대해 할 말**.
