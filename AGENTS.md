...
| 기계가 본다 (`장식으로 위계를 만들지 않는다` · `평가를 시각으로 말하지 않는다` · `subject 를 색으로 가르지 않는다` · `세로 리듬이 자 셋에서만 나온다` · `list 의 열은 균일하고 표만 옆으로 굴린다` · `선은 끊기지 않고 이어서 변형된다` · `글 자리는 한 칸씩 끊어서 찍는다` · `움직임을 끈 사람에게도 자리가 선다`) |
| --- |
...
| 선이 `d` 보간으로 **이어서** 변형되는지, 그 좌표가 산출물의 꼭짓점과 같은지, `d` 를 못 받는 자리에 꺾은선 교대가 남는지 — 둘 중 하나만 서는지 |
| 글 자리가 **한 칸씩** 뛰는지 (한 칸 100ms · 다 찍고 1000ms · 지울 때 한 칸 50ms), 낱말 길이가 여럿인지, 자리 폭이 가장 긴 낱말과 같은지, 커서가 멈춘 동안에만 깜빡이는지 |
| 조용한 자리에서 **칠·테두리·획을 가진 자리가 사라지는지** — 굴리기만 멈추면 밑그림이 남는다 |
...
| **도는 속도가 눈에 거슬리지 않는가** — 어느 값이 맞는지는 기계가 못 잰다. 판정은 속도가 `--spin`·`--pass`·`--type` 하나에서만 나오는지까지만 본다 |
| **선의 변형이 매끄럽게 이어지는가** — 좌표가 짝지어 움직여도 중간에 선이 접히거나 축 밖으로 새는지는 눈이 안다. 어느 순간에도 한 모양으로 굳어 보이지 않는가 |
| **글 자리가 타이핑으로 읽히는가** — 한 칸씩 뛰는 것이 로딩 막대가 아니라 「쓰고 있다」로 보이는가. 100ms·1000ms·50ms 가 편한 속도인가 |
