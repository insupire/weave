...
`schema/` 넷 — `weave-common`(어휘) · `weave-template`(분석 템플릿) · `weave-valueset`(값 한 벌) · `weave-render-args`(화면 상태 둘).
...
- **없다는 말은 뜻마다 다르게 선다.** 값이 설 자리면 **표기**(`—` 값 없음 · `·` 없음), 값이 설 리 없는 자리면 **글**(「항목 없음」·축 아래 줄 머리). 빈 칸으로 두지 않고, 뜻은 `aria-label`·`title` 이 글로 갖는다. **「그리지 못한다」는 표기로 줄지 않는다** — 값의 문제가 아니라 템플릿과 값이 어긋난 신호다.
- **전부 비어 있는 값 한 벌도 자리를 얻는다.** 모든 facet 이 「값 없음」이라고 말하고 왜인지는 주석이 말한다.
- **렌더 인자는 값이 말할 수 없는 것뿐이다.** `focus` 와 `previousFocus` — 앱만 아는 상호작용 이력이다. 값 한 벌이 이미 아는 것(명단·이름·차례)은 인자가 아니다.
...
| 없다는 표기가 **뜻마다 다르고** 비어 있지 않으며 이름표로 뜻을 갖는지, 「그리지 못한다」가 표기로 뭉개지지 않는지 |
