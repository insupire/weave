...
- **주석은 붙은 자리가 동작을 정한다. 갈래가 아니다.** 템플릿에 붙은 것은 모든 subject 에 같아 **늘 보이고**, 값 한 벌의 필드에 붙은 것은 subject 마다 달라 **표시 뒤에서 열린다.** 갈래 넷은 뜻만 가른다 — 보이는 방식을 갈래로 만들지 않는다.
- **템플릿 필드 주석은 1~120자 한 줄, 최대 셋.** 길어지면 값 옆이 아니라 facet 의 말이다. `description` 은 분석에게 주는 추출 지시라 화면에 안 나온다.
