...
| 기계가 본다 (`장식으로 위계를 만들지 않는다` · `평가를 시각으로 말하지 않는다` · `subject 를 색으로 가르지 않는다` · `세로 리듬이 자 셋에서만 나온다` · `list 의 열은 균일하고 표만 옆으로 굴린다` · `빈 자리의 움직임은 빛줄기 하나다` · `그림 자리가 추세로 읽히지 않는다` · `움직임을 끈 사람에게도 자리가 선다`) |
| --- |
