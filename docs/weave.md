...
- **주석 넷은 붙은 자리에 그대로 보인다.** 템플릿 facet 의 것, 값 한 벌 facet 의 것, 값의 것.
  facet 에 붙은 것은 본문에 펼치고, **필드에 붙은 것은 표시를 세워 가리키면 연다** — 갈래 표시는 값 옆에 늘 서 있다.
...
- **그리는 것과 말이 맞는다.** 겹치는 facet 은 전원의 주석을, `focus` 를 따라 바뀌는 facet 은
  **그리고 있는 subject 의 주석만** 낸다. 보이지 않는 subject 의 말은 무엇에 대한 말인지 알 수 없다.
