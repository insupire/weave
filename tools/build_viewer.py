...
def html_source(samples_js: str) -> str:
    """``samples_js`` 는 디스크가 아니라 방금 만든 글을 받는다. --check 가 묵은 파일을 보면 안 된다."""
    script = "\n\n".join(
        [
...
def main(argv: list[str] | None = None) -> int:
    ...
    wanted = {OUT_SAMPLES: samples_js, OUT_HTML: html_source(samples_js)}
    ...
    if stale:
        ...
        return 1
    if args.check:
        print("viewer.html 이 viewer/ · samples/ 와 같다")
    return 0
